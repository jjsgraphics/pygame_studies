...
def main():
  ...
  while run:
    ...
    if keys[pygame.K_w] and p1.y - P1_VEL >= 0:
      p1.y -= P1_VEL
    if keys[pygame.K_a] and p1.x - P1_VEL >= 0:
      p1.x -= P1_VEL
    if keys[pygame.K_s] and p1.y + P1_VEL + p1.height <= HEIGHT:
      p1.y += P1_VEL
    if keys[pygame.K_d] and p1.x + P1_VEL + p1.width <= WIDTH:
      p1.x += P1_VEL
